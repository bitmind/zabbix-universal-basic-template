...
* **Universal**: Can be applied to any host; it will display data from any already linked templates. Works alongside all your existing templates and templated dashboards.
 * **Host-centric**: optimized for single-host analysis (for multi-host visualization, use global Dashboard, Latest Data, Problems section, ...)
...
* **Quick access**: The ":: All data" dashboard is designed to always be the first tab in the list.
...
### Web monitoring sub-page
Disaplys all web scenarios configured on the host.

## Requirements
* **Zabbix 7.0.x** (should be compatible with 7.2 and 7.4, but not tested).
...
## Limitations
* **Standard widgets limitations**: The dashboard currently lacks some useful information (interface availability, proxy status, number of unsupported LLDs, links to unsupported objects, and a complete Inventory view) as these cannot be displayed using standard Zabbix 7.0 widgets. This may be updated in future versions.

* **Performance impact**: The template includes several "Internal type" items to gather host metadata (unsupported items, interfaces, maintenance status): on large installation please evaluate the impact on your Zabbix instance before applying it to thousands of hosts. Simply disable those items at template level if you don't want them.
